...
class RadioButton(gtk.Button):
    ...
    def expose_radio_button(self, widget, event):
        ...
        # Draw Background.
        if self.motion: 
            draw_radial_round(cr, x+w/2, y+h/2, w/2+1, theme.get_dynamic_shadow_color("hSeparator").get_color_info())
        ...
        if self.motion:
            draw_radial_round(cr, x+w/2, y+h/2, w/2, theme.get_dynamic_shadow_color("hSeparator").get_color_info())
