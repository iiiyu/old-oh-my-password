...
class Menu(object):
    ...
    def __init__(self, items, font_size=DEFAULT_FONT_SIZE, opacity=0.9, menu_pos=MENU_POS_TOP_CENTER, 
        ...
        # Init menu window.
        self.menu_window = gtk.Window(gtk.WINDOW_TOPLEVEL)
        self.menu_window.set_decorated(False)
        ...
        self.menu_window.add_events(gtk.gdk.KEY_RELEASE_MASK)
        self.menu_window.add_events(gtk.gdk.POINTER_MOTION_MASK)
        self.menu_window.add_events(gtk.gdk.BUTTON_PRESS_MASK)
        self.menu_window.add_events(gtk.gdk.BUTTON_RELEASE_MASK)
        self.menu_window.add_events(gtk.gdk.FOCUS_CHANGE_MASK)
        self.menu_window.connect("size-allocate", draw_window_shape)
        self.menu_window.connect("focus-out-event", lambda w, e: self.hide())
        draw_window_background(self.menu_window, BACKGROUND_IMAGE, 0.9, "menuMask")    
        ...
        self.menu_window.add(self.item_align)
    ...
    def get_menu_icon_info(self, items):
        ...
        for item in items:
            if item:
                ...
                if item_icon:
                    have_icon = True
                    icon_width = theme.get_dynamic_pixbuf(item_icon).get_pixbuf().get_width()
                    icon_height = theme.get_dynamic_pixbuf(item_icon).get_pixbuf().get_height()
                    break
...
class MenuItem(object):
    ...
    def create_separator_item(self):
        ...
        self.item_box = HSeparator(
            theme.get_dynamic_shadow_color("hSeparator").get_color_info(),
            self.item_padding_x, 
            self.item_padding_y)
    ...
    def expose_menu_item(self, widget, event, item_icon, item_content):
        ...
        font_color = theme.get_dynamic_color("menuFont").get_color()
        
        # Draw select effect.
        if widget.state in [gtk.STATE_PRELIGHT, gtk.STATE_ACTIVE]:
            # Draw background.
            draw_vlinear(cr, rect.x, rect.y, rect.width, rect.height, 
                         theme.get_dynamic_shadow_color("menuItemSelect").get_color_info(),
                         MENU_ITEM_RADIUS)
            
            # Set font color.
            font_color = theme.get_dynamic_color("menuSelectFont").get_color()
            
        # Draw item icon.
        pixbuf = None
        pixbuf_width = 0
        if item_icon:
            pixbuf = theme.get_dynamic_pixbuf(item_icon).get_pixbuf()
