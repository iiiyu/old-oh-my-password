...
class DynamicTreeView(object):
    ...
    def expose_callback(self, widget, event):
        '''Expose callback.'''
        if self.ticker != theme.ticker:
            self.ticker = theme.ticker
            self.update_color()
...
class Theme(object):
    ...
    def __init__(self):
        ...
        # Scan theme files.
        theme_dir = self.get_image_dir()
        for root, dirs, files in os.walk(theme_dir):
            for filepath in files:
                path = (os.path.join(root, filepath)).split(theme_dir)[1]
    ...
    def get_image_dir(self):
        '''Get theme directory.'''
        return "../theme/%s/image/" % (self.theme_name)
    ...
    def get_animation_dir(self):
        '''Get theme directory.'''
        return "../theme/%s/animation/" % (self.theme_name)
    ...
    def get_theme_dir(self):
        '''Get theme directory.'''
        return "../theme/%s/" % (self.theme_name)
