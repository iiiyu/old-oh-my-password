...
class PreviewWindow(object):
    ...
    def __init__(self, image_arry):
        '''Init.'''        
        self.preview_window = gtk.Window(gtk.WINDOW_TOPLEVEL)
        self.preview_window.set_position(gtk.WIN_POS_CENTER)
        self.preview_window.set_modal(True)
        self.preview_window.set_decorated(False)
        ...
        self.image_sum  = len(image_arry)-1
        self.ticker = 0
        self.scrollnum = 0
        ...
        self.preview_window.connect("size-allocate", draw_window_shape)
        
        draw_window_background(self.preview_window, BACKGROUND_IMAGE)
        self.preview_window.add(self.main_box)
    ...
    def move_window(self, widget, event):
        if event.button == 1:
            self.preview_window.begin_move_drag(event.button, 
                                                int(event.x_root), 
                                                int(event.y_root),
                                                event.time)
